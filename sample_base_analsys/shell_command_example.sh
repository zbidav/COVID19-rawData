...
#General script to combine STAR output
python STARstatsV2.py -r STAR_dir/ -o STAR_summery_dir/ --plot -po STAR_summery_dir/  --force


#SALMON
mkdir -p SALMON/ResultFiles
salmon1.4.0/salmon-latest_linux_x86_64/bin/salmon quant -l A -1 <(cat SRR13250339_1.fastq) -2 <(cat SRR13250339_2.fastq) -o SALMON/SRR13250339 -i /SALMON/index/hg38 -g /SALMON/index/hg38/gencode_v32.transcriptToGeneID.tab -p 8 
...
## General script to combine SALMON 
Rscript "uniteSalmonFIles.R" -i SALMON/ResultFiles/ -o SALMON/ --wide_genes --wide_samples

#calculation of ISG score (please see refence)
## 'Calculate the ISG core score (mean absolute deviation modified Z-score) as specified in article (using their code): Tumor-derived IFN triggers chronic pathway agonism and sensitivity to ADAR loss (https://doi.org/10.1038/s41591-018-0302-5)
Rscript ZMAD_ScoreCalc_V2.R -i "SALMON/SalmonTPM.wideSamples.csv" -o "SALMON/" --log2 
...
#REDI-Known
# the file Orshay_AG_list.bed obtained from: Gabay, O., Shoshan, Y., Kopel, E. et al. Landscape of adenosine-to-inosine RNA recoding across human tissues. Nat Commun 13, 1184 (2022). https://doi.org/10.1038/s41467-022-28841-4
cp Orshay_AG_list.bed known/SRR13250339/SRR13250339/SRR13250339known_sites_bed.txt

python REDITOOLS/REDItools-1.0.4/reditools/REDItool.py -i STAR/SRR13250339/SRR13250339Aligned.sortedByCoord.out.bam -f hg38.fa -l known/SRR13250339/SRR13250339/SRR13250339_sites_bed.txt -v 1 -n 0.001 -r 1 -T5-5 -c 20 -Q 33 -o known/SRR13250339/SRR13250339/SRR13250339_Folder
